...
const UserChat = () => {
  ...
  const { user_id, first_name, last_name } = useSearchParams();
  ...
  const fetchMessages = async () => {
    const response = await axios.get(
      `${BASE_URL}/user/messages/${user_id}/${userInfo?.id}`
    );
  ...
  const onSend = async (newMessages) => {
    const response = await axios.post(`${BASE_URL}/user/messages/create`, {
      sender_id: user_id,
