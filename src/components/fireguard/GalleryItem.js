...
const GalleryItem = ({ item }) => {
  const images = item.uploaded_image;
  const videos = item.uploaded_video;
  ...
  return (
    <TouchableOpacity
      onPress={() =>
        router.navigate("UploadedImages", { images, videos, user })
      }
    >
      <Card
        mode="contained"
      ...
      >
        ...
        {images?.length>0 ? (
          <Card.Cover
            source={{ uri: `${BASE_URL2}/${images[0].uri}` }}
            className=""
          />
        ) : (
          <Card.Cover
            source={{ uri: `${BASE_URL2}/${videos[0].thumbnail}` }}
            className=""
          />
        )}
        <Card.Actions className="absolute right-0 bottom-0">
          <View className="bg-primary w-10 h-10 flex items-center justify-center rounded-tl-lg rounded-br-lg shadow-md">
            <Text className="font-Poppins_600 text-xl text-orange-50">
              {parseInt(images.length)+parseInt(videos.length)}
            </Text>
          </View>
        </Card.Actions>
      </Card>
